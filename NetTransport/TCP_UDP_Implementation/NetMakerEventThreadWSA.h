...
{
  // ибо тогда надо будет делать блокировку если использовать list.size()
...
protected:
  ...
  // в Engine вызывается lock - поток отрабатывает по событиям сокетов
  // потом unlock - и в параллельном потоке происходит разблокировка
  // идет добавление или удаление сокетов
  GCS mMutexSleep;
