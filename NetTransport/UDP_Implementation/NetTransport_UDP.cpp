...
void TNetTransport_UDP::SendUnchecked()
{
  ...
  TMultiMapIP_Ptr_It bit = mMultiMapWaitCheck.begin();
  TMultiMapIP_Ptr_It eit = mMultiMapWaitCheck.end();
  while(bit!=eit)
	{
...
{
  set<TIP_Port>::iterator bit = setIP_port.begin();
  set<TIP_Port>::iterator eit = setIP_port.end();
  while(bit!=eit)
  {
