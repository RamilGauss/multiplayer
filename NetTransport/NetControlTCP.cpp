...
bool TNetControlTCP::Open( unsigned short port, unsigned char numNetWork )
{
  ...
  mSocketUp   = mDevice.Open(false, port, numNetWork);// мы как клиент
	...
	GetMakerEvent()->Remove(mSocketDown);
	GetMakerEvent()->Add(mSocketDown, this);
	GetMakerEvent()->SetTypeEvent(mSocketDown, lEvent);

	GetMakerEvent()->Remove(mSocketUp);
	GetMakerEvent()->Add(mSocketUp, this);
	GetMakerEvent()->SetTypeEvent(mSocketUp, lEvent);

  return (mSocketUp!=-1) && (mSocketDown!=-1);
}
//------------------------------------------------------------------------------
...
void TNetControlTCP::Send(unsigned int ip, unsigned short port, TBreakPacket& bp)
{
  ...
  int sock = GetSocketByIpPort(ip, port);

	// добавить заголовки в начало  - 1 байт под начало + 3 байта - размер
	// и конец 1 байт
...
//----------------------------------------------------------------------------------
int TNetControlTCP::GetSocketByIpPort( unsigned int ip, unsigned short port)
{
	return 0;
}
//----------------------------------------------------------------------------------
