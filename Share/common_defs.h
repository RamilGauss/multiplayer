...
// функция перевода из 10-й СС в двоичную целого положительного числа
inline int DecToBinary( int dec )
{
  int result = 0; 
  unsigned short numPos = 0; // текущий номер разряда, в который записывается остаток от деления  
  while( dec > 0 )
  {
    int i = dec % 2;
    if( i )
...
//-----------------------------------------------------------------------------
// Наличия соединения по каналу связи
typedef enum 
{
  scNo      = 0,  // нет связи
  scWaiting =-1,  // не определено (установка соединения)
  scOk      = 1   // связь есть
} TStateConnect;
//-----------------------------------------------------------------------------
...
// Специальные значения возвращаемые методами чтения
#define RR_TIMEOUT	0   /* данных нет, операция завершена по таймауту */
#define RR_ERROR		-1  /* ошибка выполнения операции                 */
