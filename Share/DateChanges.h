...
14.08.2013:
  BOOST_FOREACH нельзя работать с map и set, если нужно менять содержимое внутри.
	Проблема решена: использовать BOOST_FOREACH(TMapClass::value_type& bit, mMap)
	                                bit.second->Func();
21.08.2013: 
  А после 9 сентября появится возможность
  отправки резюме в Облгаз: Артем Александрович Косогоров, "kosogorov@vladoblgaz.ru"
...
26.09.2013:
	...
	Есть вместо 3 функций использовать 5, заменить GetServerDeveloperTool
