...
15) буфер для обмена между GUI и клиентом в виде списка -
23) "Живая стена" записи для развлечения при ожидании - 
26) Класс буферизации для DirectX -
27) Передавать ID карты при инициализации боя -
...
35) Стрим для мышки 1:10 секунды, отправка из DX - 

38) Пакет корректировки для боя. После входа, после начала боя - 
39) Обработчик для загрузки карты -
40) Содержимое пакета для начала боя - ID карты, список танков{ник, тип танка, комплект танка - башня и пушка} - 

41) проблема совместимости загрузки карты на клиенте и сервере -
42) effect для DirectX на TVisualDX -
43) Stream NULL LoadMap - стрим при загрузке карты - 
44) Структура Main в ManagerDirectX:
void TManagerDiretcX::Work()
{
  Refresh(); // освежить серверные данные - вызов также когда окно неактивно(из DXUT, см. DXUTRender3DEnvironment9() ), 
  // причем освежить только события, стрим же просто удалить из кольца
  Calc();    // расчет предсказателем координат и ориентации
  Optimize();// оптимизация: выкинуть то что невидно
  Render();  // собственно рендер силами DirectX
}
47) все изменения пакетов переделать под mo_realloc (см. как в TA_In_Fight)- 
48) Переделать балансер. 
while(если есть еще танки cnt>=2*cnt_in_command)
{
  1 Добавить в комнату танки кол-вом 2*cnt_in_command
  2 Разослать инфу 
}
49) Задача номер один: доделать TRoom
