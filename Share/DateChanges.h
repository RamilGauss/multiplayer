...
06.11.2012:
 ...
 Виден только один  сопособ - DXUT Native GUI. Но прежде надо продумать архитектуру с учетом источников
...
12.11.2012:
 - Была идея создания TankLib(классы, которые характерны для игры танки), но считаю в этом необходимости.
...
84) ManagerResource - Shader + Texture - 
85) ShaderStack - 
86) Camera - то ли в ManagerDirectX то ли в ManagerObjectCommon - 
87) Camera находится в ManagerCommonObject
...
90) Управление потоком событий окна
91) Загрузка карты
//----------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------
    INFO
...
По окончанию загрузки клиент обязан послать пакет на сервер о готовности принять корректирующий пакет(инфо о разрушенных объектах ...).
  ...
  Камера
  mat4 view, proj;
  proj.Perspective(60.0f, 1024.0f/768.0f, 0.5f, 200.0f);

  //Update camera orientation
  float yyy = 1.0f-fabs(sin(-mouse.y/200.0f));
  view.LookAt(mainCamPos, vec3(cos(mouse.x/200.0f)*yyy, sin(-mouse.y/200.0f), sin(mouse.x/200.0f)*yyy) + mainCamPos, vec3(0.0, 1.0, 0.0));

  //Update camera position
  mat4 viewT = view.TransposeRotation();
//----------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------
    BUG:
