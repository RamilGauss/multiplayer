...
//----------------------------------------------------------------------------------------------------
void TModelDX::Draw(unsigned int state,             //                              (ќт ObjectDX)
                    D3DXMATRIXA16* mArrMatrixSubset,//кол-во совпадает с mCntEffect (ќт ObjectDX)
                    D3DXMATRIXA16* mWorld,// где и как расположен объект            (ќт ObjectDX)
                    D3DXMATRIXA16* mView, // расположение и ориентаци€ камеры       (от ManagerDirectX)
                    D3DXMATRIXA16* mProj) // проектирование на плоскость экрана     (от ManagerDirectX)
{
  mWorldViewProjection = (*mWorld) * (*mView) * (*mProj);
  //1 ¬ыбрать по Ћќƒу mesh
  // расчет рассто€ни€
  float dist = GetDist(mWorld,mView);
  //2 Ќастроить pEffect на координаты и ориентацию - передать в pEffect матрицу
  // если mArrMatrixSubset==NULL, то координаты и ориентаци€ частей модели неизменны
  //3 ¬ыбрать по состо€нию subSet
...
bool TModelDX::Load(LPCWSTR strFilenameData)
{
...
#if 1
  pLoadModel = new TLoaderModelDXTest(m_pd3dDevice);//в основном дл€ экспериментов
#else
  ...
  {
    USES_CONVERSION;
    GlobalLoggerDX.WriteF_time("Ќе удалось загрузить модель с HDD: %s.\n",W2A(strFilenameData));
    delete pLoadModel;
    return false;
  }
  //-------------------------------------------------------------------------------------------
...
bool TModelDX::SetupEffectDX(TEffectDX *pEffect,ILoaderModelDX::TDefGroup * pDefGroup)
{
  ...
  // оптимизаци€ структуры данных
  int cntAdjacency = pEffect->pMesh->GetNumFaces() * 3;
  DWORD* aAdjacency = new DWORD[cntAdjacency];
  if( aAdjacency == NULL )
  {
    GlobalLoggerDX.WriteF_time("Ќехватка пам€ти.SetupEffectDX().\n");
    return false;
  }
