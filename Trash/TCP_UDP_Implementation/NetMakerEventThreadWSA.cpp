...
void TNetMakerEventThreadWSA::Engine()
{
  ...
  while(!flgNeedStop)
  {
    ...
    unlockSleep();
		...
		ht_msleep(0);// дать отработать другим потокам - вообще дурацкая идея строить сетевой движок на множестве потоков!
  }
  flgActive = false;
}
//----------------------------------------------------------------------------------
...
{
  boost::thread work_thread(boost::bind(&TNetMakerEventThreadWSA::Engine, this));
