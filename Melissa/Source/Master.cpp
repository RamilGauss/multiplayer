  ...
  BOOST_FOREACH(unsigned int id, l_id_client)
  {
    ...
    if(pClient->GetGroupID()!=-1)
    {
      ...
      GetLogger()->Get(STR_NAME_MELISSA)->
        WriteF_time("TMaster::TryCreateGroup() клиент(%u) уже находится в группе id=%u.\n",
    ...
    if(pClient->GetState()==TClientForMasterPrivate::eLogin)
    {
      // генерация ошибки
      GetLogger()->Get(STR_NAME_MELISSA)->
        WriteF_time("TMaster::TryCreateGroup() клиент(%u) находится в процессе авторизации.\n",id);
      return false;
    }
  }
                             ...
                             void* resForClient, int sizeResClient)
{
  ...
  if(GetClientByKey(id_client))
  {
    // генерация ошибки
    GetLogger()->Get(STR_NAME_MELISSA)->
      WriteF_time("TMaster::SetResultLogin() регистрация клиента по уже заданному ключу=%u.\n",id_client);
    return;
  }
...
unsigned int TMaster::GetSlaveSessionByGroup(unsigned int id_group)
{
  ...
  {
    GetLogger()->Get(STR_NAME_MELISSA)->
      WriteF_time("TMaster::GetSlaveSessionByGroup() пустая группа id=%u.\n",id_group);
    return -1;
  }
...
void TMaster::NeedContextLoginClient(unsigned int id_session)
{
  if(GetClientByIDSessionWait(id_session))
  {
    // внутренняя ошибка
    GetLogger()->Get(STR_NAME_MELISSA)->
      WriteF_time("TMaster::LoginClient() повторная попытка авторизации.\n");
    return;
  }
...
void TMaster::NeedContextLoginSlave(unsigned int id_session)
{
  if(GetSlaveByIDSession(id_session))
  {
    // внутренняя ошибка
    GetLogger()->Get(STR_NAME_MELISSA)->
      WriteF_time("TMaster::LoginSlave() повторная попытка авторизации.\n");
    return;
  }
